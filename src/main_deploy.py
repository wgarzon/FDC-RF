...
def installLibraries(nodes):
    ...
    exec_cmd_as_root("pip3 install Cython -U scikit-learn==0.22 pandas matplotlib && pip3 install pyzmq && pip3 install gdown", nodes)
...
def main():
    ...
    gksites = {"nancy":"gros","nantes":"econome", "rennes":"parasilo"}
    if len(sys.argv) > 1: port =  int(sys.argv[1])
    if len(sys.argv) > 2: nodes_number = int(sys.argv[2])+1
    if addCentral:nodes_number=nodes_number+1
    ...
    Put(allNodes, ["~/.ssh/id_rsa","~/.ssh/id_rsa.pub"], "/root/.ssh/", connection_params = {'user': 'root'}).run()
    ...
    if addCentral:
        print("Central Site Node:", centralNode, "***********\n")
        Put(centralNode, centralFiles, remFolder, connection_params = {'user': 'root'}).run()
        print("ok: Copying files into central node")
        nodes = allSites.copy()
        nodes.append(mainCsite)

    print("Main Controller Node:", mainCsite)
    print("Site Nodes:", allSites)

    #run initialization tasks only the first time
    if deploy or initSites:
    ...
    #Move aggregated dataset to central node
    if addCentral:
        mv_model_to_central(centralNode, mainCsite)
        print("ok: Main Controller move models to central node")
        mv_data_to_central(centralNode, mainCsite)
        print("ok: Main Controller move data to central node")
